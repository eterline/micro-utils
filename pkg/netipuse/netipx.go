...
func RangeOfPrefix(p netip.Prefix) PoolRange {
	p = p.Masked()
	if !p.IsValid() {
		return PoolRange{}
	}
	return PoolRangeFrom(p.Addr(), PrefixLastIP(p))
}
...
// PoolRange represents an inclusive range of IP addresses
...
// PoolRangeFrom returns an PoolRange from from to to.
// It does not allocate.
func PoolRangeFrom(from, to netip.Addr) PoolRange {
	return PoolRange{
		from: from.WithZone(""),
		to:   to.WithZone(""),
	}
...
// MustParsePoolRange calls ParsePoolRange(s) and panics on error.
// It is intended for use in tests with hard-coded strings.
func MustParsePoolRange(s string) PoolRange {
	r, err := ParsePoolRange(s)
	if err != nil {
		panic(err)
	}
	return r
...
func (r PoolRange) String() string {
	...
	if !r.from.IsValid() || !r.to.IsValid() {
		return "zero PoolRange"
	}
	return "invalid PoolRange"
}
...
// The IP range is expected in a form accepted by ParsePoolRange.
// It returns an error if *r is not the PoolRange zero value.
func (r *PoolRange) UnmarshalText(text []byte) error {
	if *r != (PoolRange{}) {
		return errors.New("refusing to Unmarshal into non-zero PoolRange")
	}
	if len(text) == 0 {
		return nil
	}
	var err error
	*r, err = ParsePoolRange(string(text))
	return err
}

// IsZero reports whether r is the zero value of the PoolRange type.
...
func (r PoolRange) Valid() bool { return r.IsValid() }
...
func (r PoolRange) less(other PoolRange) bool {
...
// space.
func (r PoolRange) entirelyBefore(other PoolRange) bool {
...
// other.
func (r PoolRange) coveredBy(other PoolRange) bool {
...
// edges of other.
func (r PoolRange) inMiddleOf(other PoolRange) bool {
...
func (r PoolRange) overlapsStartOf(other PoolRange) bool {
	return lessOrEq(r.from, other.from) && r.to.Less(other.to)
}

// overlapsEndOf returns whether r entirely overlaps the end of
// other, but not all of other.
func (r PoolRange) overlapsEndOf(other PoolRange) bool {
	return other.from.Less(r.from) && lessOrEq(other.to, r.to)
}

// mergePoolRanges returns the minimum and sorted set of IP ranges that
// cover r.
func mergePoolRanges(rr []PoolRange) (out []PoolRange, valid bool) {
	...
	case 0:
		return nil, true
	case 1:
		return []PoolRange{rr[0]}, true
	}

	sort.Slice(rr, func(i, j int) bool { return rr[i].less(rr[j]) })
	out = make([]PoolRange, 1, len(rr))
	out[0] = rr[0]
	for _, r := range rr[1:] {
		prev := &out[len(out)-1]
		switch {
...
// AppendPrefixes is an append version of PoolRange.Prefixes. It appends
...
func (r PoolRange) prefixFrom128AndBits(a uint128, bits uint8) netip.Prefix {
	var ip netip.Addr
	if r.from.Is4() {
		bits -= 12 * 8
		ip = a.IP4()
